...
async function downloadFromUrl(
  url: string,
  outputPath: string,
): Promise<{ success: boolean; endpointCount: number }> {
  ...
  try {
    ...
    if (
      ...
      log(
        "Downloaded content does not appear to be a valid OpenAPI specification",
        "error",
      );
      return { success: false, endpointCount: 0 };
    }
...
async function downloadFromGitHub(
  ...
  try {
    ...
    const result =
      await $`gh api /repos/${repo}/contents/${filePath} -H "Accept: application/vnd.github.raw" --jq .`
        .nothrow()
        .quiet();
...
async function getConfigInteractive(): Promise<OpenAPIConfig> {
  ...
  console.log(
    "  \x1B[36m1)\x1B[0m URL     — Download from an HTTP endpoint (Swagger, localhost, etc.)",
  );
  console.log(
    "  \x1B[36m2)\x1B[0m GitHub  — Download from a GitHub repository",
  );
  console.log(
    "  \x1B[36m3)\x1B[0m Local   — Copy from a file on your machine\n",
  );
  ...
  if (choice === "2") {
    ...
    return {
      ...
      branch,
      filePath,
      specFile: detectSpecFilename(filePath),
    };
  }
  ...
  if (choice === "3") {
    ...
    return {
      source: "local",
      localPath,
      specFile: detectSpecFilename(localPath),
    };
  }
...
async function syncOpenAPI(
  ...
  switch (config.source) {
    case "url":
      return downloadFromUrl(config.url!, outputPath);
    case "github":
    ...
    case "local":
      return copyFromLocal(config.localPath!, outputPath);
    default:
...
async function generateTypes(specFilePath: string): Promise<boolean> {
  ...
  const result =
    await $`bunx openapi-typescript ${specFilePath} -o ${TYPES_FILE}`
      .nothrow()
      .quiet();
...
if (urlArg) {
  ...
  config = {
    source: "local",
    localPath: fileArg,
    specFile: detectSpecFilename(fileArg),
  };
} else if (useConfig) {
...
if (endpointCount > 0) {
  config.endpointCount = endpointCount;
}
await saveConfig(config);
